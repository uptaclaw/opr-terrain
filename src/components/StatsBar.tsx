  ...
  return (
    <>
      <div data-testid="stats-bar" className="mb-4 flex flex-wrap items-center gap-3">
        ...
        {/* Auto Placement split button */}
        <div className="flex items-stretch overflow-hidden rounded-2xl border border-cyan-400/40">
          <button
            type="button"
            onClick={onGenerate}
            className="bg-cyan-400 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300"
          >
            {hasExistingPieces ? 'Re-generate' : 'Generate'}
          </button>
          <button
            type="button"
            onClick={onOpenAutoPlacement}
            aria-label="Auto placement settings"
            className="border-l border-cyan-500/40 bg-cyan-400 px-2.5 py-2.5 text-slate-950 transition hover:bg-cyan-300"
          >
