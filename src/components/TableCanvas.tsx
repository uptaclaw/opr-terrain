...
export function TableCanvas({
  ...
  const selectedPiece = terrainPieces.find((piece) => piece.id === selectedPieceId) ?? null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <desc>
          Responsive wargaming table canvas with a 1-inch grid, deployment zones, selectable
          terrain pieces, and drag-and-drop editing.
        </desc>
        ...
        <g aria-hidden="true">
          ...
          {selectedPiece && selectedPiece.shape.kind !== 'circle' ? (
            (() => {
              const centerX = tableX + selectedPiece.x;
              const centerY = tableY + selectedPiece.y;
              const handleX = centerX;
              const handleY = Math.max(tableY + 1.2, centerY - selectedPiece.collisionRadius - 1.7);

              return (
                <g>
                  <line
                    x1={centerX}
                    y1={centerY - selectedPiece.collisionRadius * 0.75}
                    x2={handleX}
                    y2={handleY + 0.55}
                  ...
                  <g
                    ...
                    onMouseDown={(event) => {
                      event.preventDefault();
                      event.stopPropagation();
                      onRotateHandleMouseDown?.(selectedPiece.id, event as unknown as ReactMouseEvent<SVGGElement>);
                    }}
